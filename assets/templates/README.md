...
| `raw_duel_opening_60.png` | 决斗场「第 N 回」顶栏中的 60 开局读秒 | fight | 用户提供的 `替身2222.mp4`（游戏客户区裁取） |
...
`raw_duel_opening_60.png` 是可缩放的开局边界证据：它匹配决斗场「第 N 回」顶栏中的 **60** 读秒；命中后会清空上一回合的冷却（保留本局累计次数），并在 60 及其开场动画期间只校准豆数，避免特效把空豆短暂照亮（例如 3→4→3）时误计替身。它不根据忍者皮肤或豆子颜色猜测开局，也不替代后续真实掉 1 豆的确认。
