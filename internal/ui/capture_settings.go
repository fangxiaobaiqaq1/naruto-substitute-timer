...
// captureProviderHasNoSeparateProbe identifies providers whose first-run
// validation is an explicit confirmation of the exact draft, not an image or
// SDK probe. Keep SDK-backed providers out of this path.
func captureProviderHasNoSeparateProbe(method string) bool {
	return method == "auto" || method == capture.MethodPrintWindow
}

// confirmNoSeparateProbeDraft is deliberately unavailable to SDK-backed
// providers. Its success records only that the exact draft was confirmed; it
// does not claim that an image was captured or validated.
func confirmNoSeparateProbeDraft(method string, draft config.CaptureConfig) (config.CaptureConfig, bool) {
	if !captureProviderHasNoSeparateProbe(method) {
		return config.CaptureConfig{}, false
	}
	return draft, true
}

type onboardingValidationKind uint8

const (
	onboardingValidationNone onboardingValidationKind = iota
	onboardingValidationNoSeparateProbe
	onboardingValidationImageProbe
)

func onboardingValidationKindForProvider(method string) onboardingValidationKind {
	if captureProviderHasNoSeparateProbe(method) {
		return onboardingValidationNoSeparateProbe
	}
	return onboardingValidationImageProbe
}

// onboardingDraftReady requires both the exact draft and the provider-specific
// validation path. In particular, no-probe confirmation can never unlock MuMu
// SDK or Leidian ADB.
func onboardingDraftReady(firstRun, validated bool, kind onboardingValidationKind, validatedDraft, draft config.CaptureConfig) bool {
	return !firstRun || (validated && kind == onboardingValidationKindForProvider(captureProvider(draft)) && reflect.DeepEqual(validatedDraft, draft))
}

func onboardingValidationText(firstRun, validated bool, kind onboardingValidationKind, validatedDraft, draft config.CaptureConfig) string {
	if !firstRun {
		return "保存会写入当前草稿，并在下一次采集时请求切换。"
	}
	method := captureProvider(draft)
	if !validated || kind == onboardingValidationNone {
		if captureProviderHasNoSeparateProbe(method) {
			return "首次设置：保存已锁定。此后端无需单独截图自检；请点击“测试当前选择（不保存）”确认当前草稿后保存。"
		}
		return "首次设置：保存已锁定。MuMu SDK 和雷电 ADB 必须对当前草稿自检成功后才能保存。"
	}
	if kind != onboardingValidationKindForProvider(method) || !reflect.DeepEqual(validatedDraft, draft) {
		return "首次设置：保存已锁定。当前草稿或后端在验证后已改变，请重新验证当前选择。"
	}
	if kind == onboardingValidationNoSeparateProbe {
		return "首次设置：当前草稿已确认无需单独截图自检，可以保存并应用。"
	}
	return "首次设置：当前草稿已通过截图连接自检，可以保存并应用。"
}
...
// captureSettingsLayout keeps the primary actions outside the scrollable
// provider and advanced-settings content.
func captureSettingsLayout(scrollContent, footer fyne.CanvasObject) fyne.CanvasObject {
...
func (s *session) captureControls(w fyne.Window) fyne.CanvasObject {
	...
	refreshValidation := func() {
		ready := onboardingDraftReady(s.firstRun, s.onboardingProbeOK, validationKind, s.onboardingProbe, draft)
		validationStatus.SetText(onboardingValidationText(s.firstRun, s.onboardingProbeOK, validationKind, s.onboardingProbe, draft))
		if saveSelected != nil {
			if s.firstRun && !ready {
	...
	invalidateOnboardingValidation := func() {
		if s.firstRun {
			s.onboardingProbeOK = false
			validationKind = onboardingValidationNone
			refreshValidation()
		}
	}
	refreshDraft := func() {
		draftLabel.SetText("待保存：" + captureTargetText(draft))
		refreshValidation()
	}
	...
	applyConfig := func(next config.CaptureConfig, label string) {
		if !onboardingDraftReady(s.firstRun, s.onboardingProbeOK, validationKind, s.onboardingProbe, next) {
			validationStatus.SetText(onboardingValidationText(s.firstRun, s.onboardingProbeOK, validationKind, s.onboardingProbe, next))
			dialog.ShowInformation("请先验证当前选择", "首次配置中，自动和 PrintWindow 需要确认当前草稿；MuMu SDK 和雷电 ADB 需要成功读取当前目标的截图。修改后端或目标后请重新验证。", w)
			return
		}
		s.mu.Lock()
		cfg := s.cfg
	...
	selectProvider := func(name string) {
		...
		// A provider switch is always a new first-run choice, even if the user
		// later switches back to a previously confirmed draft.
	...
	refresh.OnTapped = func() {
		...
		go func() {
			...
			fyne.Do(func() {
				if base.Err() != nil || s.stopped() || !captureDiscoveryCanApply(w, s.captureView, now, generation, scannedProvider, captureProviderFromLabel(provider.Selected)) {
					return
				}
				refresh.Enable()
				if err != nil {
	...
	// Editing a visible target field invalidates first-run validation before a
	// save attempt, so the fixed footer never continues to claim readiness.
	markTargetEdited := func(string) { invalidateOnboardingValidation() }
	mumuRoot.OnChanged = markTargetEdited
	mumuNumber.OnChanged = markTargetEdited
	ldRoot.OnChanged = markTargetEdited
	ldConsole.OnChanged = markTargetEdited
	ldADB.OnChanged = markTargetEdited
	ldPackage.OnChanged = markTargetEdited
	ldConnect.OnChanged = func(bool) { invalidateOnboardingValidation() }
	oldLDIndexChanged := ldIndex.OnChanged
	ldIndex.OnChanged = func(value string) {
		oldLDIndexChanged(value)
		invalidateOnboardingValidation()
	}
	oldLDSerialChanged := ldSerial.OnChanged
	ldSerial.OnChanged = func(value string) {
		oldLDSerialChanged(value)
		invalidateOnboardingValidation()
	}
	refreshValidation()
	...
	test = widget.NewButton("测试当前选择（不保存）", func() {
		...
		if method == capture.MethodLeidianADB {
			...
			go func() {
				...
				fyne.Do(func() {
					if base.Err() != nil || s.stopped() || s.captureView != w || captureProviderFromLabel(provider.Selected) != capture.MethodLeidianADB {
						return
					}
					test.Enable()
					if result.Error != "" {
					...
					refreshDraft()
					if s.firstRun {
						s.onboardingProbe, s.onboardingProbeOK = draft, true
						validationKind = onboardingValidationImageProbe
						refreshValidation()
					}
					status.SetText(probeSuccessText("雷电 ADB", result.Width, result.Height))
				})
			}()
			return
		}
		if captureProviderHasNoSeparateProbe(method) {
			if s.firstRun {
				// This is a deliberate exact-draft confirmation, not an image
				// probe. confirmNoSeparateProbeDraft rejects SDK-backed providers.
				s.onboardingProbe, s.onboardingProbeOK = confirmNoSeparateProbeDraft(method, draft)
				validationKind = onboardingValidationNoSeparateProbe
				refreshValidation()
				status.SetText("当前后端无需单独截图自检：已确认当前草稿，可保存并应用。未读取或验证图像。")
				return
			}
		...
		invalidateOnboardingValidation()
		test.Disable()
		status.SetText("正在通过独立进程检查 MuMu SDK 连接与截图（测试未保存草稿；运行采集沿用已保存/已应用目标）…")
		executable := s.executablePath
		if executable == "" {
			executable, _ = os.Executable()
		}
		go func() {
			ctx, cancel := context.WithTimeout(base, mumu.HelperTransactionTimeout)
			defer cancel()
			manual := next.Selection == "manual" || (next.Selection != "auto" && (next.InstallDir != "" || next.Instance != 0 || next.DLLPath != ""))
			result, runErr := support.ProbeSDK(ctx, executable, mumu.Options{InstallDir: next.InstallDir, DLLPath: next.DLLPath, Instance: next.Instance, DisplayID: next.DisplayID, Package: next.Package, Manual: manual})
			fyne.Do(func() {
				if base.Err() != nil || s.stopped() || s.captureView != w || captureProviderFromLabel(provider.Selected) != capture.MethodMuMuSDK {
					return
				}
				test.Enable()
				if runErr != nil {
				...
				status.SetText(probeSuccessText("MuMu SDK", result.Width, result.Height))
			})
		}()
	})
	...
	// Advanced settings contain only manual overrides. The primary actions are
	// kept in the fixed footer below. The inactive section is collapsed, not hidden,
	// so switching provider never makes one adapter look unsupported or second-class.
	...
	actionRow := container.NewGridWithColumns(2, test, saveSelected)
	footer := container.NewVBox(widget.NewSeparator(), container.NewPadded(container.NewVBox(validationStatus, actionRow)))
	scrollContent := container.NewVScroll(container.NewVBox(
		sectionCard("选择模拟器进程", container.NewVBox(providerForm, applied, draftLabel, status, refresh, container.NewStack(spacer, choices))),
		muManual,
		ldManual,
	))
	return captureSettingsLayout(scrollContent, footer)
}
